<section class="tf-section tf-about">
    <div class="container">
        <div class="row">
            <div class="ol-xl-7 col-lg-7 col-md-12">
                <div class="wrap-image ps-re">
                    <img src="/site/assets/images/background/bg-aboutus-2.png" alt="" class="bg1-about">
                    <div class="fx m-b32 align-fl-end">
                        <div class="image m-r30 wow fadeIn animated" data-wow-delay="0.3ms" data-wow-duration="1200ms">
                            <img src="/site/assets/images/original/about-test.jpg" width="550px" alt="Images">
                        </div>
                        <div class="image wow fadeIn animated" data-wow-delay="0.3ms" data-wow-duration="1200ms">
                        <img src="/site/assets/images/original/calistung1-test.jpg" width="550px" alt="Images">
                        </div>
                    </div>
                    <div class="image p-l70"><img src="/site/assets/images/original/mapelsd1-test.jpg" width="550px" alt="Images"></div>
                </div>                        
            </div>
            <div class="col-xl-5 col-lg-5 col-md-12 col-12">
                <div class="sc-about-2 p-t19">
                    <div class="title-heading">
                        <div class="sub-heading clr-pri-3 f-mulish">
                            <span class="inner-sub st-2">Tentang Kami</span>
                            <svg  xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="77" height="30" viewBox="0 0 77 30">
                                <g >
                                <image  width="77" height="30" xlink:href="data:img/png;base64,"/>
                                </g>
                            </svg>
                        </div>
                        <h2 class="title clr-pri-2">Syameela Kids
                        </h2>
                    </div>
                    <div class="inner">
                        <p class="wrap f-mulish clr-pri-4  wow fadeIn animated" data-wow-delay="0.3ms" data-wow-duration="1000ms">
                            Syameela Kids Menyediakan program privat calistung yaitu program pendampingan belajar<B> BACA, TULIS, HITUNG.</B> Program ini dipersembanhkan untuk anak-anak Pra Sekolah hingga SD awal yang masih belum mampu menguasai <B>Calistung</B> dengan baik.</p>
                        <h4 class="title-line">Kenapa Memilih Syameela Kids</h4>
                        <ul>
                            <li class="st-1 fx"><i class="fas fa-check"></i><p class="clr-pri-2">Belajar Privat</p></li>
                            <li class="st-4 fx"><i class="fas fa-check"></i><p class="clr-pri-2">Modul Lengkap</p></li>
                            <li class="st-2 fx"><i class="fas fa-check"></i><p class="clr-pri-2">Perkembangan Anak</p></li>
                            <li class="st-1 fx"><i class="fas fa-check"></i><p class="clr-pri-2">Belajar Sesuai Kebutuhan</p></li>
                            <li class="st-3 fx"><i class="fas fa-check"></i><p class="clr-pri-2">Pengajar Kompeten</p></li> 
                        </ul>

                        <!-- <a href="testimonials.html" class="fl-btn st-11">
                            <span class="inner">Baca Lagi</span>
                        </a> -->
                    </div>
                </div>
            </div>
        </div>  
    </div>
</section>