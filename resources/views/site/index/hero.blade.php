<section class="tf-slider-1">
    <div class="overlay"></div>
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                <div class="slider-1">
                        <div class="themesflat-carousel clearfix" data-margin="30" data-item="1" data-item2="1" data-item3="1" data-item4="1" data-auto="false">
                            <div class="owl-carousel owl-theme none dots-none">
                                <div class="item-slider-1">
                                    <div class="box-content">
                                        <div class="sub clr-pri-2">Spesialis Bimbel Privat</div>
                                        <div class="title clr-pri-2">CALISTUNG & MAPEL SD</div>
                                        <div class="box-custom">
                                            <div class="wrap clr-pri-1">Syameela Kids</div>
                                            <svg data-name="Hero Area" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="112" height="71" viewBox="0 0 112 71">
                                                <g>
                                                    <image width="112" height="71" xlink:href="data:img/png;base64,"/>
                                                </g>
                                                </svg>
                                        </div>
                                        <ul>
                                            <li class="st-1 fx"><i class="fas fa-check"></i><p class="clr-pri-2">CALISTUNG</p></li>
                                            <li class="st-2 fx"><i class="fas fa-check"></i><p class="clr-pri-2">MENGAJI</p></li>
                                            <li class="st-3 fx"><i class="fas fa-check"></i><p class="clr-pri-2">MAPEL SD</p></li>
                                        </ul>
                                        <div class="box-btn">
                                            <!-- <a href="contact/" class="fl-btn st-12">
                                                <span class="inner">Kontak Kami</span>
                                            </a> -->
                                            <a href="about.html" class="fl-btn st-1">
                                                <span class="inner">Selengkapnya</span>
                                            </a>
                                        </div>
                                    </div>
                                    <div class="box-feature">
                                        <div class="image">
                                            <img src="site/assets/images/original/slider1-test.jpg" alt="Image">
                                        </div>
                                    </div>
                                </div>
                                <!-- <div class="item-slider-1">
                                    <div class="box-content">
                                        <div class="sub clr-pri-2">Spesialis Bimbel Privat</div>
                                        <div class="title clr-pri-2">CALISTUNG & MAPEL SD</div>
                                        <div class="box-custom">
                                            <div class="wrap clr-pri-1">Syameela Kids</div>
                                            <svg data-name="Hero Area" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="112" height="71" viewBox="0 0 112 71">
                                                <g>
                                                    <image width="112" height="71" xlink:href="data:img/png;base64,"/>
                                                </g>
                                                </svg>
                                        </div>
                                        <ul>
                                            <li class="st-1 fx"><i class="fas fa-check"></i><p class="clr-pri-2">Outdoor Games</p></li>
                                            <li class="st-2 fx"><i class="fas fa-check"></i><p class="clr-pri-2">Sport Activites</p></li>
                                            <li class="st-3 fx"><i class="fas fa-check"></i><p class="clr-pri-2">Nutritious Foods</p></li>
                                        </ul>
                                        <div class="box-btn">
                                            <a href="#" class="fl-btn st-12">
                                                <span class="inner">Hubungi Kami</span>
                                            </a>
                                            <a href="#" class="fl-btn st-1">
                                                <span class="inner">Baca Lagi</span>
                                            </a>
                                        </div>
                                    </div>
                                    <div class="box-feature">
                                        <div class="image">
                                            <img src="site/assets/images/common/slider-1.png" alt="Image">
                                        </div>
                                    </div>
                                </div> -->
                            </div>
                            
                        </div><!--/.themesflat-carousel-->
                </div><!--/.slider-2-->
            </div>
        </div>
    </div>
</section>